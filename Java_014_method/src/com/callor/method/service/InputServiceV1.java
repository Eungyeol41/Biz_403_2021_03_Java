...
public class InputServiceV1 {
	 ...
	 * 매우 엄격하게 필요한 클래스이다. 사용하는 방법에 따라, 오류가 발생하거나 심지어 컴퓨터 시스템을 멈추는 현상까지 발생할 ㅜㅅ 있다 이러한
	 * 클래스는 사용할 때 상당한 주의가 필요하다
	 */

	// Scaner 클래스를 main() method가 아닌 Service 등의 클래스에서 사용할 때는 클래스 영역에 선언을 하고 클래스의
	// 생성자에서 생성(초기화)하는 것이 좋다
	 ...
	 * return된 값에 따라 
	 * 		null이 return되면 그냥 다시 return해버리고 
	 * 		정수값이 return되면 start, end 값 범위에 있는 값인지 유효성 검사를 하고 범위를 벗어나면 다시 입력하도록 반복한다
	 */
	...
	/*
	 * String title 매개변수를 받고 정수값이나 QUIT가 입력되는 지 확인을 하고
	 * 잘못 입력한 값이 있으면 다시 입력하도록 하는 method
	 */
