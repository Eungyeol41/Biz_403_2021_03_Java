 ...
 * 하지만 배열은 그 구조 상 사용이 다소 불편할 때가 많다
 * Java에서는 배열을 사용해야 하는 프로젝트에서 List 클래스를 사용한 객체를 생성하여 진행을 한다
 */
public class IolistServiceV1 {
	/*
	 * 매입매출 데이터들을 사용하여 연산을 수행할텐데 매입매출 데이터들의 묶음을 List형 객체를 사용하겠다
	 * 
	 * List형 객체에 포함된 (개별) 데이터는 IolistVO형으로 사용하겠다
	 * 
	 * 낱개 데이터는 IolistVO클래스 형으로 사용하고 그 데이터들을 묶어서 List형으로 연산수행을 하겠다라는 선언
	 * 
	 * iolist를 private으로 선언한 이유 혹시 외부에서 iolist데이터에 접근하여 값을 저장하는 일을 방지하기 위함이다 V1클래스를
	...
	// return type이 명확히 결정되지 않았을 때는 void형으로 작성을 한다
