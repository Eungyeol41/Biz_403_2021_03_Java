...
public class ScoreServiceV5 {
	 ...
	 * 몇 명의 학생, 일부 과목 점수만 입력마치고 잠시 업무를 멈추고 싶을 때 '현재 코드는 불가능하다'
	 *  => 일부만 입력하고 업무를 중단하는 기능을 넣자
	 */
	public void makeScores() {
		
		// 과목의 개수만큼 점수을 담을 임시 배열 선언
		...
		for(int i = 0; i < intKor.length; i++) {
			...
			for(int j = 0; j < subject.length; j++) {
				...
				/*
				 * 입력업무 중에 -1을 입력했을 때는 입력업무를 중단하고 main()으로 되돌아가고 싶다
				 * 이 때 break;를 사용하면 for()가 겹쳐있기 때문에 원하는대로 코드가 작동되지 않는다.
				 * 이럴 경우 return type이 void이지만 return명령을 실행하여 method 코드
				 */
				if(scores[j] == -1) {
					return;
				}
