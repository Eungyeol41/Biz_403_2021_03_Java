...
public class ScoreServiceV1A {
	...
	// 사용할 준비가 되어있지 않다 == 생성, 초기화가 안 되었다
	Scanner scan;
	
	// 학생 수만큼 과목점수를 저장할 배열 선언
	int[] intKor;
	int[] intEng;
	int[] intMath;
	...
	public void makeScores() {
		...
		// 만약 정상적인 범위( 0 ~ 100 )를 입력하면 반복문을 종료하고, 다음 코드가 실행되도록 한다
		while(true) {
	...
	// method의 코드 내에 "return 값" 형식의 코드가 반드시 있어야 함을 선언하는 것
	
	// return type이 void형(type)일 경우는 return 명령문이 생략되거나 단독으로 사용할 수 있다
	
	// return type이 void형이 아닌 경우 return 명령문은 반드시 코드 내에 적절한 위치에 있어야 하고,
