...
public class NumberServiceV1A {
	/*
	 * Random 수를 생성하여 3의 배수이면 그 수를 아니면 null을 return하라
	 * 		==> 반드시 어떤 값을 return하라
	 ...
	 *  	int와 Integer가 있다(정수형 type)
	 *  	
	 *  return의 두 번째 조건에 "아니면 null값을 return하라'라는 지시가 있으므로 int형은 사용할 수 없다.
	 * 
	 * void 키워드
	 * 		method에 return 명령이 꼭 없어도 되는 경우
	 * 
	 * 이 method는 정수형 3의 배수 또는 null값을 return해야 하기 때문에 Integer형 return type을 설정해야 한다
	 * 		void 대신 Integer를 사용한다
	 * 
	 * void 대신 Integer를 사용하면 반드시 return 명령문과 return할 값(대상)이 코드에 있어야 한다.
	 * 	아직 method에 return문이 없기 때문에 오류가 발생
	 * 
	 * method에 오류가 나는 것을 없애기 위해 return을 추가한다
	 * 이 method는 return 명령문만으로 처리가 되지 않고 반드시 return할 값(대상)이 있어야 한다.
	 * 
	...
	public Integer inputNum() {
		...
		if(num2 % 3 == 0) {
			 ...
			 * 별다른 문제를 일으키거나 하지는 않지만 실제 코드가 작동될 때는 int 형을 Integer wrapper 클래스 형으로 변환하는 과정이 일어난다
			 */
			return num2;
		}
		return null;
